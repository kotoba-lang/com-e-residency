(ns e-residency.main
  "Kotodama WASM entrypoint for the E_residency clean-room actor (L4) — Clojure port.
  ...
  facts (`e_residency.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "e_residency")
(def tier "L4")
...
(def entity-specs
  [{:entity "Filing"      :plural "filings"      :id-prefix "eresiden_fil"
    :fields [:filerId :scheme :period :status]   :required [:scheme :period]
    :coerce {}                       :refs {}}
   {:entity "Citizen"     :plural "citizens"     :id-prefix "eresiden_cit"
    :fields [:nationalId :jurisdiction]          :required [:jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Service"     :plural "services"     :id-prefix "eresiden_ser"
    :fields [:name :department]                  :required [:name :department]
    :coerce {}                       :refs {}}
   {:entity "Application" :plural "applications" :id-prefix "eresiden_app"
    :fields [:citizenId :serviceId :status]      :required [:status]
    :coerce {}                       :refs {:citizenId "Citizen" :serviceId "Service"}}
   {:entity "Dataset"     :plural "datasets"     :id-prefix "eresiden_dat"
    :fields [:name :publisher :updatedAt]        :required [:name :publisher]
    :coerce {}                       :refs {}}
   {:entity "Identity"    :plural "identities"   :id-prefix "eresiden_ide"
...
(defn emit-facts
  "EAVT facts for one record: {\"e_residency.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "e_residency-compat" :tier tier :entities entities} 200])
...
(defn start! [] :e_residency-compat/ready)
